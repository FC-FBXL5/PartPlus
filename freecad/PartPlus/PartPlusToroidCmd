...
# -*- coding: utf-8 -*-
'''
This Tool creates a toroid shape, either a PartDesign feature or
...
class ToroidShape(BaseShape):
    '''
    Adds a toroid shape
    '''
    def __init__(self, obj):
        super(ToroidShape, self).__init__(obj)
        '''
        Adds properties to the object
        '''
        print("#--- ToroidShape, init(), obj: ", obj, "\n")
        ...
        self.addToroidProperties(obj)

        self.obj = obj
        obj.Proxy = self

    def addToroidProperties(self, obj):
        '''
        Creates the Toroid parameters including default values required
        for the initial build (without checking their existence)
        '''
        SHAPE_TYPES = (
            "Feature_Union",
            "Feature_Difference",
        ...
        addAngleProperty(
            ...
            translate(
                "App::Property",
                "Reverse angle around the rotation axis"
            ),
            30.0,
            "ParametersDistribution"
        )
        ...
        addBoolProperty(
            ...
            translate(
                "App::Property",
                "Creates a hollow toroid from a closed profile"
            ),
            False,
            "ParametersBaseProfile"
        )
        addBoolProperty(
            ...
            translate(
                "App::Property",
                "Toggles between vertical and horizontal rotation axis"
            ),
            True,
            "ParametersDistribution"
        )
    ...
    def execute(self, obj):
        ...
        new_shape = self.generateToroidShape(obj)

        self.finishNewShape(obj, new_shape)

    def generateToroidShape(self, obj):
        '''
        ...
        forward_angle = obj.ForwardAngle.Value
        reverse_angle = obj.ReverseAngle.Value
        symmetric_shape = obj.Symmetric
        two_sided_shape = obj.TwoSided
        reverse_shape = obj.Reverse
        vertical_axis = obj.VerticalAxis
        ...
        length = 10.0  # arbitrary value to calculate faces from open wires

        #- Set axis - should be adjustible in the future
        axis_position = App.Vector (0, 0, 0)  # Origin

        if vertical_axis:
            axis_direction = profile_y_axis
        else:
            axis_direction = profile_x_axis
        ...
        profile_face.rotate(axis_position, axis_direction, -reverse_angle)

        toroid_shape = profile_face.revolve(
            axis_position,
            axis_direction,
...
if App.GuiUp:
    ...
    class ToroidShapeViewProvider(ViewProviderPartPlus):
        '''
        Individual view provider features for toroid shape objects.
        '''
...
Console.PrintLog('freecad/PartPlus/PartPlusToroidCmd.py\n')
